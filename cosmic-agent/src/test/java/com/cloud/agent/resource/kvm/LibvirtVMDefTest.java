...
public class LibvirtVMDefTest extends TestCase {
    ...
    public void testInterfaceEtehrnet() {
        ...
        ifDef.defEthernet("targetDeviceName", "00:11:22:aa:bb:dd", NicModel.VIRTIO);
    ...
    public void testInterfaceDirectNet() {
        final LibvirtVmDef.InterfaceDef ifDef = new LibvirtVmDef.InterfaceDef();
        ifDef.defDirectNet("targetDeviceName", null, "00:11:22:aa:bb:dd", NicModel.VIRTIO,
                "private");

        final String expected = "<interface type='" + GuestNetType.DIRECT + "'>\n"
    ...
    public void testChannelDef() {
        final RngBackendModel backendModel = RngBackendModel.RANDOM;
        ...
        assertEquals(def.getRngModel(), RngModel.VIRTIO);
    }

    public void testWatchDogDef() {
        LibvirtVmDef.WatchDogDef def = null;

        def = new LibvirtVmDef.WatchDogDef(WatchDogAction.RESET,
                WatchDogModel.I6300ESB);
        assertEquals(def.getAction(), WatchDogAction.RESET);
        assertEquals(def.getModel(), WatchDogModel.I6300ESB);

        def = new LibvirtVmDef.WatchDogDef(WatchDogAction.POWEROFF,
                WatchDogModel.DIAG288);
        assertEquals(def.getAction(), WatchDogAction.POWEROFF);
        assertEquals(def.getModel(), WatchDogModel.DIAG288);
    }
