...
public class VirtualRoutingResource {
    ...
    private final Map<String, Lock> _vrLockMap = new HashMap<>();
    ...
    private ExecutionResult applyConfigToVR(final String routerAccessIp, final ConfigItem c, final int timeout) {
        if (c instanceof FileConfigItem) {
            ...
            return this._vrDeployer.createFileInVR(routerAccessIp, configItem.getFilePath(), configItem.getFileName(), configItem.getFileContents());
        } else if (c instanceof ScriptConfigItem) {
            final ScriptConfigItem configItem = (ScriptConfigItem) c;
            return this._vrDeployer.executeInVR(routerAccessIp, configItem.getScript(), configItem.getArgs(), timeout);
        }
    ...
    private Answer execute(final CheckRouterCommand cmd) {
        final ExecutionResult result = this._vrDeployer.executeInVR(cmd.getRouterAccessIp(), VRScripts.RVR_CHECK, null);
        if (!result.isSuccess()) {
    ...
    public boolean connect(final String ipAddress, final int retry, final int sleep) {
        for (int i = 0; i <= retry; i++) {
            ...
            try {
                ...
                final InetSocketAddress addr = new InetSocketAddress(ipAddress, this._port);
                sch.connect(addr);
                return true;
