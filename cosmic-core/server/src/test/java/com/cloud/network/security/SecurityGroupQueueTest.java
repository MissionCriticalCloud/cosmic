...
public class SecurityGroupQueueTest extends TestCase {
    ...
    public void testNumJobsEqToNumVms1() {
        ...
        final Thread[] pThreads = new Thread[numProducers];

        final Producer[] producers = new Producer[numProducers];
        int numProduced = 0;
    ...
    protected void testNumJobsEqToNumVms2(final int numProducers, final int maxVmId) {
        queue.clear();
    ...
    public void testDequeueOneJobAgain() {
        ...
        final Thread cThread = new Thread(new Consumer(1));
        cThread.start();
        try {
            cThread.join();
    ...
    protected void _testDequeueOneJob(final int numConsumers, final int numProducers, final int maxVmId) {
        ...
        final Thread[] pThreads = new Thread[numProducers];
        final Thread[] cThreads = new Thread[numConsumers];

        final Consumer[] consumers = new Consumer[numConsumers];
        final Producer[] producers = new Producer[numProducers];
    ...
    public static class Producer implements Runnable {
        ...
        public Producer(final int maxVmId) {
    ...
    public static class Consumer implements Runnable {
        ...
        public Consumer(final int numJobsToDequeu) {
        ...
        @Override
        public void run() {
            List<SecurityGroupWork> result = new ArrayList<>();
            try {
