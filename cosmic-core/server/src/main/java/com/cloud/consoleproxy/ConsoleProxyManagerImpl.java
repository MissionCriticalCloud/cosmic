...
public class ConsoleProxyManagerImpl extends SystemVmManagerBase implements ConsoleProxyManager, VirtualMachineGuru, ResourceStateAdapter {
    ...
    public ConsoleProxyInfo assignProxy(final long dataCenterId, final long vmId) {
        ...
        final KeystoreVO ksVo = this._ksDao.findByName(ConsoleProxyManager.CERTIFICATE_NAME);
        if (proxy.isSslEnabled() && ksVo == null) {
            logger.warn("SSL enabled for console proxy but no server certificate found in database");
        }

        if (this._staticPublicIp == null) {
            return new ConsoleProxyInfo(proxy.isSslEnabled(), proxy.getPublicIpAddress(), this._consoleProxyPort, proxy.getPort(), this._consoleProxyUrlDomain);
        } else {
            return new ConsoleProxyInfo(proxy.isSslEnabled(), this._staticPublicIp, this._consoleProxyPort, this._staticPort, this._consoleProxyUrlDomain);
        }
    }

    public ConsoleProxyVO doAssignProxy(final long dataCenterId, final long vmId) {
        ConsoleProxyVO proxy = null;
        final VMInstanceVO vm = this._instanceDao.findById(vmId);
        ...
        if (this._allocProxyLock.lock(ACQUIRE_GLOBAL_LOCK_TIMEOUT_FOR_SYNC)) {
            try {
                if (vm.getProxyId() != null) {
                    proxy = this._consoleProxyDao.findById(vm.getProxyId());
                    ...
                    if (proxy != null) {
                        if (!isInAssignableState(proxy)) {
                            ...
                            if (this._consoleProxyDao.getProxyActiveLoad(proxy.getId()) < this._capacityPerProxy || hasPreviousSession(proxy, vm)) {
        ...
        if (vm.getProxyId() == null || vm.getProxyId().longValue() != proxy.getId()) {
            this._instanceDao.updateProxyId(vmId, proxy.getId(), DateUtil.currentGMTTime());
        }

        proxy.setSslEnabled(this._sslEnabled);
        if (this._sslEnabled) {
            proxy.setPort(443);
        } else {
            proxy.setPort(80);
    ...
    private ConsoleProxyAllocator getCurrentAllocator() {
        ...
        for (final ConsoleProxyAllocator allocator : this._consoleProxyAllocators) {
            return allocator;
        }
    ...
    public boolean configure(final String name, final Map<String, Object> params) throws ConfigurationException {
        ...
        this._consoleProxyUrlDomain = configs.get(Config.ConsoleProxyUrlDomain.key());
        if (this._sslEnabled && (this._consoleProxyUrlDomain == null || this._consoleProxyUrlDomain.isEmpty())) {
        ...
        this._capacityScanInterval = NumbersUtil.parseLong(value, DEFAULT_CAPACITY_SCAN_INTERVAL);

        this._capacityPerProxy = NumbersUtil.parseInt(configs.get("consoleproxy.session.max"), DEFAULT_PROXY_CAPACITY);
        this._standbyCapacity = NumbersUtil.parseInt(configs.get("consoleproxy.capacity.standby"), DEFAULT_STANDBY_CAPACITY);
        this._proxySessionTimeoutValue = NumbersUtil.parseInt(configs.get("consoleproxy.session.timeout"), DEFAULT_PROXY_SESSION_TIMEOUT);

        value = configs.get("consoleproxy.port");
        if (value != null) {
            this._consoleProxyPort = NumbersUtil.parseInt(value, ConsoleProxyManager.DEFAULT_PROXY_VNC_PORT);
        }

        value = configs.get(Config.ConsoleProxyDisableRpFilter.key());
        if (value != null && value.equalsIgnoreCase("true")) {
            this._disableRpFilter = true;
        }

        value = configs.get("secondary.storage.vm");
        if (value != null && value.equalsIgnoreCase("true")) {
            this._useStorageVm = true;
        }

        logger.info("Console proxy max session soft limit : " + this._capacityPerProxy);
        logger.info("Console proxy standby capacity : " + this._standbyCapacity);
        ...
        this._listener = new ConsoleProxyListener(new VmBasedAgentHook(this._instanceDao, this._hostDao, this._configDao, this._ksMgr, this._agentMgr, this._keysMgr));
        this._agentMgr.registerForHostEvents(this._listener, true, true, false);

        this._itMgr.registerGuru(VirtualMachineType.ConsoleProxy, this);

        //check if there is a default service offering configured
        final String cpvmSrvcOffIdStr = configs.get(Config.ConsoleProxyServiceOffering.key());
        if (cpvmSrvcOffIdStr != null) {
            this._serviceOffering = this._offeringDao.findByUuid(cpvmSrvcOffIdStr);
            if (this._serviceOffering == null) {
                try {
                    this._serviceOffering = this._offeringDao.findById(Long.parseLong(cpvmSrvcOffIdStr));
                } catch (final NumberFormatException e) {
        ...
        this._loadScanner.initScan(STARTUP_DELAY, this._capacityScanInterval);
        this._resourceMgr.registerResourceStateAdapter(this.getClass().getSimpleName(), this);

        this._staticPublicIp = this._configDao.getValue("consoleproxy.static.publicIp");
        if (this._staticPublicIp != null) {
            this._staticPort = NumbersUtil.parseInt(this._configDao.getValue("consoleproxy.static.port"), 8443);
        }
    ...
    public boolean stop() {
        ...
        this._allocProxyLock.releaseRef();
    ...
    public boolean finalizeVirtualMachineProfile(final VirtualMachineProfile profile, final DeployDestination dest, final ReservationContext context) {
        ...
        final ConsoleProxyVO vm = this._consoleProxyDao.findById(profile.getId());
    ...
    public boolean finalizeDeployment(final Commands cmds, final VirtualMachineProfile profile, final DeployDestination dest, final ReservationContext context) {
        ...
        final ConsoleProxyVO proxy = this._consoleProxyDao.findById(profile.getId());
        final Zone zone = dest.getZone();
        final List<NicProfile> nics = profile.getNics();
        computeVmIps(proxy, zone, nics);
        this._consoleProxyDao.update(proxy.getId(), proxy);
        return true;
    }
    ...
    public void finalizeExpunge(final VirtualMachine vm) {
        final ConsoleProxyVO proxy = this._consoleProxyDao.findById(vm.getId());
    ...
    public boolean canScan() {
        ...
        final List<StoragePoolVO> upPools = this._storagePoolDao.listByStatus(StoragePoolStatus.Up);
    ...
    public void onScanStart() {
        ...
        this._zoneProxyCountMap = new HashMap<>();
        final List<ConsoleProxyLoadInfo> listProxyCounts = this._consoleProxyDao.getDatacenterProxyLoadMatrix();
        for (final ConsoleProxyLoadInfo info : listProxyCounts) {
            this._zoneProxyCountMap.put(info.getId(), info);
        }

        this._zoneVmCountMap = new HashMap<>();
        final List<ConsoleProxyLoadInfo> listVmCounts = this._consoleProxyDao.getDatacenterSessionLoadMatrix();
        for (final ConsoleProxyLoadInfo info : listVmCounts) {
            this._zoneVmCountMap.put(info.getId(), info);
        }
    }
    ...
    public boolean isPoolReadyForScan(final Long pool) {
        ...
        final List<ConsoleProxyVO> l = this._consoleProxyDao.getProxyListInStates(VirtualMachine.State.Starting, VirtualMachine.State.Stopping);
        if (l.size() > 0) {
    ...
    public boolean isZoneReady(final Map<Long, ZoneHostInfo> zoneHostInfoMap, final long dataCenterId) {
        ...
        if (zoneHostInfo != null && isZoneHostReady(zoneHostInfo)) {
            ...
            final TemplateDataStoreVO templateHostRef = this._vmTemplateStoreDao.findByTemplateZoneDownloadStatus(template.getId(), dataCenterId, Status.DOWNLOADED);
    ...
    public Pair<AfterScanAction, Object> scanPool(final Long pool) {
        ...
        final ConsoleProxyLoadInfo proxyInfo = this._zoneProxyCountMap.get(dataCenterId);
        if (proxyInfo == null) {
    ...
    private boolean checkCapacity(final ConsoleProxyLoadInfo proxyCountInfo, final ConsoleProxyLoadInfo vmCountInfo) {
        ...
        if (proxyCountInfo.getCount() * this._capacityPerProxy - vmCountInfo.getCount() <= this._standbyCapacity) {
            return false;
        }
    ...
    public ConsoleProxyVO assignProxyFromStoppedPool(final long dataCenterId) {
        ...
        final List<ConsoleProxyVO> l = this._consoleProxyDao.getProxyListInStates(dataCenterId, State.Starting, State.Stopped, State.Migrating, State.Stopping);
        if (l != null && l.size() > 0) {
            return l.get(0);
        }
    ...
    private boolean allowToLaunchNew(final long dcId) {
        ...
        final List<ConsoleProxyVO> l =
                this._consoleProxyDao.getProxyListInStates(dcId, VirtualMachine.State.Starting, VirtualMachine.State.Running, VirtualMachine.State.Stopping,
                        VirtualMachine.State.Stopped, VirtualMachine.State.Migrating, VirtualMachine.State.Shutdowned, VirtualMachine.State.Unknown);

        final String value = this._configDao.getValue(Config.ConsoleProxyLaunchMax.key());
    ...
    protected Map<String, Object> createProxyInstance(final long dataCenterId, final VMTemplateVO template) throws ConcurrentOperationException {
        ...
        final List<? extends NetworkOffering> offerings =
                this._networkModel.getSystemAccountNetworkOfferings(NetworkOffering.SystemControlNetwork, NetworkOffering.SystemManagementNetwork);
        ...
        if (serviceOffering == null) {
            serviceOffering = this._offeringDao.findDefaultSystemOffering(ServiceOffering.consoleProxyDefaultOffUniqueName, ConfigurationManagerImpl.SystemVMUseLocalStorage.valueIn
                    (dataCenterId));
        }
    ...
    public ConsoleProxyManagementState getManagementState() {
        final String curentState = this._configDao.getValue(Config.ConsoleProxyManagementState.key());
    ...
    public void resumeLastManagementState() {
        ...
        if (lastState != state) {
            this._configDao.update(Config.ConsoleProxyManagementState.key(), Config.ConsoleProxyManagementState.getCategory(), lastState.toString());
        }
    }
    ...
    public ConsoleProxyVO startProxy(final long proxyVmId, final boolean ignoreRestartSetting) {
        try {
            ConsoleProxyVO proxy = this._consoleProxyDao.findById(proxyVmId);
            if (proxy.getState() == VirtualMachine.State.Running) {
                return proxy;
            }

            final String restart = this._configDao.getValue(Config.ConsoleProxyRestart.key());
            if (!ignoreRestartSetting && restart != null && restart.equalsIgnoreCase("false")) {
                return null;
            }

            if (proxy.getState() == VirtualMachine.State.Stopped) {
                this._itMgr.advanceStart(proxy.getUuid(), null, null);
                proxy = this._consoleProxyDao.findById(proxy.getId());
                return proxy;
            }
    ...
    public boolean stopProxy(final long proxyVmId) {
        ...
        try {
            this._itMgr.stop(proxy.getUuid());
            return true;
    ...
    public boolean destroyProxy(final long vmId) {
        final ConsoleProxyVO proxy = this._consoleProxyDao.findById(vmId);
        try {
            //expunge the vm
            this._itMgr.expunge(proxy.getUuid());
            proxy.setPublicIpAddress(null);
            proxy.setPublicMacAddress(null);
            proxy.setPublicNetmask(null);
            proxy.setPrivateMacAddress(null);
            proxy.setPrivateIpAddress(null);
            this._consoleProxyDao.update(proxy.getId(), proxy);
            this._consoleProxyDao.remove(vmId);
            final HostVO host = this._hostDao.findByTypeNameAndZoneId(proxy.getDataCenterId(), proxy.getHostName(), HostType.ConsoleProxy);
            if (host != null) {
                logger.debug("Removing host entry for proxy id=" + vmId);
                return this._hostDao.remove(host.getId());
            }
    ...
    private void handleResetSuspending() {
        final List<ConsoleProxyVO> runningProxies = this._consoleProxyDao.getProxyListInStates(State.Running);
        ...
        final List<ConsoleProxyVO> proxiesInTransition = this._consoleProxyDao.getProxyListInStates(State.Running, State.Starting, State.Stopping);
    ...
    private ConsoleProxyManagementState getLastManagementState() {
        final String lastState = this._configDao.getValue(Config.ConsoleProxyManagementLastState.key());
    ...
    public class VmBasedAgentHook extends AgentHookBase {
            ...
            if (state == HostStatus.Alert || state == HostStatus.Disconnected) {
                ...
                if (host.getType() == HostType.ConsoleProxy) {
                    ...
                    if (name != null && name.startsWith("v-")) {
                        ...
                        final ConsoleProxyVO proxy = ConsoleProxyManagerImpl.this._consoleProxyDao.findById(proxyVmId);
                        if (proxy == null) {
        ...
        protected HostVO findConsoleProxyHost(final StartupProxyCommand startupCmd) {
            ...
            final ConsoleProxyVO consoleProxy = ConsoleProxyManagerImpl.this._consoleProxyDao.findById(proxyVmId);
