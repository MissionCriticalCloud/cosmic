...
public class TransactionTest {
    ...
    public void testUserManagedConnection() {
        final DbTestDao testDao = ComponentContext.inject(DbTestDao.class);
        final TransactionLegacy txn = TransactionLegacy.open("SingleConnectionThread");
        ...
        try {
            ...
            final Connection checkConn = TransactionLegacy.currentTxn().getConnection();
            if (checkConn != conn) {
                Assert.fail("A new db connection is acquired instead of using old one after create sql");
            }
            testDao.update(2, 2, "Record 1");
            final Connection checkConn2 = TransactionLegacy.currentTxn().getConnection();
            if (checkConn2 != conn) {
