...
class InstallSysTemplate(object):
    ...
    def __init__(self):
        self.dictionary = dict(xenserver=('XenServer', 'vhd'), kvm=('KVM', 'qcow2'), vmware=('VMware', 'ova'))
    ...
    def parseOptions(self):
        ...
        self.parser.add_argument("-H", "--hypervisor", action="store", dest="hypervisor", help="The Hypervisor name for which template need to be installed", required="true",
                                 choices=['kvm', 'xenserver', 'vmware'])
        ...
        self.parser.add_argument("-s", "--management-secret-key", action="store", dest="managementsecretkey",
                                 help="mgmt server secret key, if you specified any when running cloudstack-setup-database, default is password", default="password")
        ...
        self.parser.add_argument("-d", "--database-host-name", action="store", dest="databasehostname", help="Database server hostname or ip, e.g localhost", default="localhost",
                                 required="true")
        ...
        self.parser.add_argument("-p", "--database-user-password", nargs='?', action="store", dest="databaseuserpassword",
                                 help="Database password. Followed by nothing if the password is empty", default="", required="true")
        ...
        self.parser.add_argument("-t", "--file-extension", action="store", dest="fileextension", help="The template file extension", default="", required="true",
                                 choices=['bz2', 'gz', 'zip'])
...
''' % msg
    ...
    def runMysql(self, query):
        try:
            ...
            mysqlCmds = ['mysql', '--user=%s' % self.databaseusername, '--host=%s' % self.databasehostname, '--password=%s' % self.databaseuserpassword, '--skip-column-names',
                         '-U', 'cloud', '-e "%s"' % query]
