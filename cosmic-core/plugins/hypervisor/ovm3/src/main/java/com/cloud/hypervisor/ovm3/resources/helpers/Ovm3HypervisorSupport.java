...
public class Ovm3HypervisorSupport {
    ...
    private final Map<String, State> vmStateMap = new HashMap<>();
    /* statemap bounces */
    private final Map<String, PowerState> powerStateMaps;
    private final Map<String, State> stateMaps;

    {
        powerStateMaps = new HashMap<>();
    ...
    public Ovm3HypervisorSupport(final Connection conn, final Ovm3Configuration ovm3config) {
        connection = conn;
        config = ovm3config;
    }

    public void revmoveVmState(final String vmName) {
    ...
    private Map<String, State> syncState(final Map<String, State> vmStateMap)
            throws Ovm3ResourceException {
        final Map<String, State> newStates;
        Map<String, State> oldStates = null;
        final Map<String, State> changes = new HashMap<>();
        try {
        ...
        synchronized (vmStateMap) {
            oldStates = new HashMap<>(vmStateMap.size());
    ...
    private Map<String, State> getAllVmStates(final Map<String, State> vmStateMap)
            throws Ovm3ResourceException {
    ...
    public void setVmStateStarting(final String vmName) {
        setVmState(vmName, State.Starting);
    }

    public void setVmState(final String vmName, final State state) {
        synchronized (vmStateMap) {
            vmStateMap.put(vmName, state);
        }
    }

    public State getVmState(final String vmName) {
        return vmStateMap.get(vmName);
    }

    public void fillHostInfo(final StartupRoutingCommand cmd) {
        try {
    ...
    public Boolean setupServer(final String key) throws IOException {
    ...
    public File getSystemVmKeyFile(final String filename) {
    ...
    private PowerState convertStateToPower(final State statePower) {
        final PowerState state = powerStateMaps.get(statePower.toString());
        return state == null ? PowerState.PowerUnknown : state;
    }

    public CheckHealthAnswer execute(final CheckHealthCommand cmd) {
    ...
    public ReadyAnswer execute(final ReadyCommand cmd) {
        try {
            final Linux host = new Linux(connection);
    ...
    public MaintainAnswer execute(final MaintainCommand cmd) {
        logger.debug("MaintainCommand");
        return new MaintainAnswer(cmd);
    }

    public FenceAnswer execute(final FenceCommand cmd) {
    ...
    public Answer execute(final GetHostStatsCommand cmd) {
        try {
            final CloudstackPlugin cSp = new CloudstackPlugin(connection);
    ...
    public CheckOnHostAnswer execute(final CheckOnHostCommand cmd) {
