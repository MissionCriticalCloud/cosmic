...
public class Ovm3HypervisorResource extends ServerResourceBase implements HypervisorResource {
     ...
     */
    private final Map<String, Xen.Vm> vmMap = new HashMap<>();
    @Inject
     ...
     * configure is called before this, does setup of the connection and gets the params.
     *
    ...
    public StartupCommand[] initialize() {
        ...
        try {
            ...
            final StartupStorageCommand ssCmd = new StartupStorageCommand();
    ...
    public PingCommand getCurrentStatus(final long id) {
      ...
      /* feels useless somehow */
            ...
            final String pong = test.echo(ping);
    ...
    /* check if we're master or not and if we can connect */
        try {
            ...
            if (!configuration.getIsTest()) {
                connection = new Connection(configuration.getAgentIp(),
                        configuration.getAgentOvsAgentPort(),
                        configuration.getAgentOvsAgentUser(),
                        configuration.getAgentOvsAgentPassword());
                connection.setHostName(configuration.getAgentHostname());
            }
            hypervisorsupport = new Ovm3HypervisorSupport(connection, configuration);
            if (!configuration.getIsTest()) {
        ...
        hypervisornetwork = new Ovm3HypervisorNetwork(connection, configuration);
        hypervisornetwork.configureNetworking();
        virtualroutingresource = new Ovm3VirtualRoutingResource(connection);
        storagepool = new Ovm3StoragePool(connection, configuration);
        storagepool.prepareForPool();
        storageprocessor = new Ovm3StorageProcessor(connection, configuration,
                storagepool);
        vmsupport = new Ovm3VmSupport(connection, configuration, hypervisorsupport,
        ...
        virtualroutingsupport = new Ovm3VirtualRoutingSupport(connection, configuration,
    ...
    @Override
    public synchronized StartAnswer execute(final StartCommand cmd) {
        ...
        try {
            ...
            final Xen.Vm vm = xen.getVmConfig();
      /* max and min ? */
      ...
      /*
       * officially CD boot is only supported on HVM, although there is a simple way around it..
       */
            ...
            if (vmSpec.getType() != VirtualMachine.Type.User) {
                ...
                final String svmPath = configuration.getAgentOvmRepoPath() + "/"
                        + ovmObject.deDash(vm.getPrimaryPoolUuid()) + "/ISOs";
                final String svmIso = svmPath + "/"
        ...
        /* fix is in cloudstack.py for xend restart timer */
          ...
          /* skip a beat to make sure we didn't miss start */
                    if (hypervisorsupport.getVmState(vmName) == null && count > 1) {
                        final String msg = "VM " + vmName + " went missing on "
                                + configuration.getAgentHostname()
                                + ", returning stopped";
                        LOGGER.debug(msg);
                        state = State.Stopped;
          ...
          /* creative fix? */
                    try {
                        final Boolean res = cSp.domrCheckSsh(controlIp);
    ...
    public StopAnswer execute(final StopCommand cmd) {
        ...
        try {
            ...
            final String repoId = ovmObject.deDash(vm.getVmRootDiskPoolId());
            final String vmId = vm.getVmUuid();
      /* can we do without the poolId ? */
            vms.stopVm(repoId, vmId);
            int tries = 30;
            while (vms.getRunningVmConfig(vmName) != null && tries > 0) {
                final String msg = "Waiting for " + vmName + " to stop";
                LOGGER.debug(msg);
                tries--;
            ...
            if (vms.getRunningVmConfig(vmName) != null) {
                final String msg = "Stop " + vmName + " failed ";
                LOGGER.debug(msg);
