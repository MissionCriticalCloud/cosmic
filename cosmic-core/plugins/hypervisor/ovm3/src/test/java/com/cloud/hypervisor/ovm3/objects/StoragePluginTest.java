...
public class StoragePluginTest {
    ...
    public void testStorageFileCreation() throws Ovm3ResourceException {
        ...
        final FileProperties file = sPt.storagePluginCreate(FSMNTUUID, NFSHOST, FILE,
                SIZE, false);
        file.getOnDiskSize();
    }
    ...
    public void testStorageFileCreationFileExistS()
            throws Ovm3ResourceException {
        con.setResult(results.errorResponseWrap("exceptions OSError:[Errno.17] File exists "
                + FILE));
        final FileProperties file = sPt.storagePluginCreate(FSMNTUUID, NFSHOST, FILE,
                SIZE, false);
        file.getSize();
    ...
    public void testStorageFileInfo() throws Ovm3ResourceException {
        con.setResult(results.simpleResponseWrapWrapper(FILECREATEXML));
        final FileProperties file = sPt.storagePluginGetFileInfo(FSMNTUUID, NFSHOST,
                FILE);
        file.getName();
        file.getUuid();
        file.getType();
    ...
    public void testDiscoverStoragePlugins() throws Ovm3ResourceException {
        ...
        for (final String plugin : sPt.discoverStoragePlugins()) {
