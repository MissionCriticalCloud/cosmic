...
public class BaremetalKickStartServiceImpl extends BareMetalPxeServiceBase implements BaremetalPxeService {
  ...
  private DomainRouterVO getVirtualRouter(Network network) {
    ...
    final DomainRouterVO vr = routers.get(0);
    if (Hypervisor.HypervisorType.KVM.equals(vr.getHypervisorType()) || Hypervisor.HypervisorType.XenServer.equals(vr.getHypervisorType())) {
  ...
  private List<String> parseKickstartUrl(VirtualMachineProfile profile) {
    final String tpl = profile.getTemplate().getUrl();
    assert tpl != null : "How can a null template get here!!!";
    final String[] tpls = tpl.split(";");
    final CloudRuntimeException err =
  ...
  private boolean preparePxeInBasicZone(VirtualMachineProfile profile, NicProfile nic, DeployDestination dest, ReservationContext context) throws AgentUnavailableException, OperationTimedoutException {
    final NetworkVO nwVO = _nwDao.findById(nic.getNetworkId());
    ...
    final BaremetalPxeVO pxeVo = sc.find();
    if (pxeVo == null) {
    ...
    final VMTemplateVO template = _tmpDao.findById(profile.getTemplateId());
    final List<String> tuple =  parseKickstartUrl(profile);

    final String ks = tuple.get(0);
    final String kernel = tuple.get(1);
    final String initrd = tuple.get(2);

    final PrepareKickstartPxeServerCommand cmd = new PrepareKickstartPxeServerCommand();
    cmd.setKsFile(ks);
    cmd.setInitrd(initrd);
    cmd.setKernel(kernel);
    cmd.setMac(nic.getMacAddress());
    cmd.setTemplateUuid(template.getUuid());
    final Answer aws = _agentMgr.send(pxeVo.getHostId(), cmd);
    if (!aws.getResult()) {
  ...
  private boolean preparePxeInAdvancedZone(VirtualMachineProfile profile, NicProfile nic, Network network, DeployDestination dest, ReservationContext context) throws Exception {
    final DomainRouterVO vr = getVirtualRouter(network);
    final List<NicVO> nics = _nicDao.listByVmId(vr.getId());
    NicVO mgmtNic = null;
    for (final NicVO nicvo : nics) {
    ...
    final String internalServerIp = _configDao.getValue(Config.BaremetalInternalStorageServer.key());
  ...
  public boolean prepare(VirtualMachineProfile profile, NicProfile nic, Network network, DeployDestination dest, ReservationContext context) {
    try {
      ...
      final IpmISetBootDevCommand bootCmd = new IpmISetBootDevCommand(BootDev.pxe);
      final Answer aws = _agentMgr.send(dest.getHost().getId(), bootCmd);
      if (!aws.getResult()) {
        s_logger.warn("Unable to set host: " + dest.getHost().getId() + " to PXE boot because " + aws.getDetails());
      }
  ...
  public BaremetalPxeVO addPxeServer(AddBaremetalPxeCmd cmd) {
    final AddBaremetalKickStartPxeCmd kcmd = (AddBaremetalKickStartPxeCmd)cmd;
    ...
    final List<HostVO> pxes = _resourceMgr.listAllHostsInOneZoneByType(Host.Type.BaremetalPxe, zoneId);
    if (!pxes.isEmpty()) {
      throw new IllegalArgumentException("Already had a PXE server zone: " + zoneId);
    }

    final String tftpDir = kcmd.getTftpDir();
    if (tftpDir == null) {
    ...
    final String guid = getPxeServerGuid(Long.toString(zoneId), BaremetalPxeType.KICK_START.toString(), ipAddress);
    ...
    final Host pxeServer = _resourceMgr.addHost(zoneId, resource, Host.Type.BaremetalPxe, params);
    if (pxeServer == null) {
      throw new CloudRuntimeException("Cannot add PXE server as a host");
    }

    final BaremetalPxeVO vo = new BaremetalPxeVO();
  ...
  public BaremetalPxeResponse getApiResponse(BaremetalPxeVO vo) {
    final BaremetalPxeResponse response = new BaremetalPxeResponse();
    response.setId(vo.getUuid());
    final HostVO host = _hostDao.findById(vo.getHostId());
    response.setUrl(host.getPrivateIpAddress());
    final PhysicalNetworkServiceProviderVO providerVO = _physicalNetworkServiceProviderDao.findById(vo.getNetworkServiceProviderId());
    response.setPhysicalNetworkId(providerVO.getUuid());
    final PhysicalNetworkVO nwVO = _physicalNetworkDao.findById(vo.getPhysicalNetworkId());
  ...
  public List<BaremetalPxeResponse> listPxeServers(ListBaremetalPxeServersCmd cmd) {
    final List<BaremetalPxeResponse> responses = new ArrayList<BaremetalPxeResponse>();
    if (cmd.getId() != null) {
      final BaremetalPxeVO vo = _pxeDao.findById(cmd.getId());
    ...
    final List<BaremetalPxeVO> vos = sc.list();
    for (final BaremetalPxeVO vo : vos) {
      responses.add(getApiResponse(vo));
    }
    return responses;
  }
