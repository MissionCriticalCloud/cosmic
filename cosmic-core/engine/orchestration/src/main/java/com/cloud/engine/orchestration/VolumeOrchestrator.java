...
public class VolumeOrchestrator extends ManagerBase implements VolumeOrchestrationService, Configurable {
    ...
    public VolumeVO allocateDuplicateVolumeVO(final Volume oldVol, final Long templateId) {
        ...
        return this._volsDao.persist(newVol);
    }
    ...
    public boolean volumeOnSharedStoragePool(final Volume volume) {
        ...
        if (poolId == null) {
            ...
            if (pool == null) {
                ...
                return pool.getScope() != ScopeType.HOST;
            }
        }
    }
    ...
    public boolean volumeInactive(final Volume volume) {
        ...
        if (vmId != null) {
            final UserVm vm = this._entityMgr.findById(UserVm.class, vmId);
            if (vm == null) {
                return true;
            }
            final State state = vm.getState();
            return state.equals(State.Stopped) || state.equals(State.Destroyed);
        }
        return false;
    }
    ...
    public String getVmNameOnVolume(final Volume volume) {
        final Long vmId = volume.getInstanceId();
        if (vmId != null) {
            final VirtualMachine vm = this._entityMgr.findById(VirtualMachine.class, vmId);
    ...
    public Pair<Pod, Long> findPod(final VirtualMachineTemplate template, final ServiceOffering offering, final DataCenter dc, final long accountId, final Set<Long> avoids) {
        for (final PodAllocator allocator : this._podAllocators) {
    ...
    public DataStoreRole getDataStoreRole(final Snapshot snapshot) {
        final SnapshotDataStoreVO snapshotStore = this._snapshotDataStoreDao.findBySnapshot(snapshot.getId(), DataStoreRole.Primary);
        ...
        final DataStore dataStore = this.dataStoreMgr.getDataStore(storagePoolId, DataStoreRole.Primary);
    ...
    public Volume migrateVolume(final Volume volume, final StoragePool destPool) throws StorageUnavailableException {
        final VolumeInfo vol = this.volFactory.getVolume(volume.getId());
        final AsyncCallFuture<VolumeService.VolumeApiResult> future = this.volService.copyVolume(vol, (DataStore) destPool);
        try {
            final VolumeService.VolumeApiResult result = future.get();
            if (result.isFailed()) {
                ...
                if (!this._snapshotDao.listByVolumeId(vol.getId()).isEmpty()) {
                    this._snapshotDao.updateVolumeIds(vol.getId(), result.getVolume().getId());
                    this._snapshotDataStoreDao.updateVolumeIds(vol.getId(), result.getVolume().getId());
                }
            }
            return result.getVolume();
        } catch (final InterruptedException e) {
    ...
    public void cleanupStorageJobs() {
        ...
        final List<AsyncJobVO> jobs = this._jobMgr.findFailureAsyncJobs(VmWorkAttachVolume.class.getName(),
    ...
    private void cleanupVolumeDuringMigrationFailure(final Long volumeId, final Long destPoolId) {
        ...
        if (volume.getState() == Volume.State.Migrating) {
            final VolumeVO duplicateVol = this._volsDao.findByPoolIdName(destPoolId, volume.getName());
            if (duplicateVol != null) {
                s_logger.debug("Remove volume " + duplicateVol.getId() + " on storage pool " + destPoolId);
                this._volsDao.remove(duplicateVol.getId());
            }
    ...
    private void cleanupVolumeDuringSnapshotFailure(final Long volumeId, final Long snapshotId) {
        this._snapshotSrv.cleanupVolumeDuringSnapshotFailure(volumeId, snapshotId);
        final VolumeVO volume = this._volsDao.findById(volumeId);
    ...
    VirtualMachineTemplate template, final Account owner, final DiskControllerType diskControllerType) {
        ...
        vol = this._volsDao.persist(vol);

        // Save usage event and update resource count for user vm volumes
        if (vm.getType() == VirtualMachineType.User) {
            this._resourceLimitMgr.incrementResourceCount(vm.getAccountId(), ResourceType.volume, vol.isDisplayVolume());
    ...
    @DB
    public VolumeInfo createVolume(final StoragePool storagePool, VolumeInfo volume, final VirtualMachineTemplate template, final DataCenter dc, final ServiceOffering offering, final DiskOffering
            diskOffering, final long size, final HypervisorType hyperType) {
    ...
    private VolumeInfo copyVolume(final StoragePool rootDiskPool, final VolumeInfo volume, final VirtualMachine vm, final VirtualMachineTemplate rootDiskTmplt, final DataCenter dcVO, final Pod pod,
        ...
        return copyVolumeFromSecToPrimary(volume, vm, rootDiskTmplt, dcVO, pod, rootDiskPool.getClusterId(), diskVO, new ArrayList<>());
    }
    ...
    protected DiskProfile createDiskCharacteristics(final VolumeInfo volume, final VirtualMachineTemplate template, final DataCenter dc, final DiskOffering diskOffering) {
        if (volume.getVolumeType() == VolumeType.ROOT && ImageFormat.ISO != template.getFormat()) {
            final TemplateDataStoreVO ss = this._vmTemplateStoreDao.findByTemplateZoneDownloadStatus(template.getId(), dc.getId(), VMTemplateStatus.DOWNLOADED);
            if (ss == null) {
    ...
    @DB
    public VolumeInfo copyVolumeFromSecToPrimary(final VolumeInfo volume, final VirtualMachine vm, final VirtualMachineTemplate template, final DataCenter dc, final Pod pod, final Long clusterId,
                                                 final DiskOffering diskOffering, final List<StoragePool> avoids) {
    ...
    public void cleanupVolumes(final long vmId) throws ConcurrentOperationException {
        ...
        final List<VolumeVO> volumesForVm = this._volsDao.findByInstance(vmId);
                ...
                for (final VolumeVO vol : volumesForVm) {
                    if (vol.getVolumeType().equals(VolumeType.ROOT)) {
                        ...
                        if (!volumeAlreadyDestroyed) {
                            VolumeOrchestrator.this.volService.destroyVolume(vol.getId());
                        } else {
                        ...
                        VolumeOrchestrator.this._volsDao.detachVolume(vol.getId());
                    }
                }
            }
        });
    ...
    public void revokeAccess(final long vmId, final long hostId) {
        ...
        if (volumesForVm != null) {
            for (final VolumeVO volumeForVm : volumesForVm) {
                final VolumeInfo volumeInfo = this.volFactory.getVolume(volumeForVm.getId());

                // pool id can be null for the VM's volumes in Allocated state
                if (volumeForVm.getPoolId() != null) {
                    final DataStore dataStore = this.dataStoreMgr.getDataStore(volumeForVm.getPoolId(), DataStoreRole.Primary);

                    this.volService.revokeAccess(volumeInfo, host, dataStore);
                }
            }
        }
    }
    ...
    public void migrateVolumes(final VirtualMachine vm, final VirtualMachineTO vmTo, final Host srcHost, final Host destHost, final Map<Volume, StoragePool> volumeToPool) {
        ...
        for (final Map.Entry<Volume, StoragePool> entry : volumeToPool.entrySet()) {
            ...
            final StoragePool destPool = (StoragePool) this.dataStoreMgr.getDataStore(storagePool.getId(), DataStoreRole.Primary);
    ...
    private Map<String, String> getDetails(final VolumeInfo volumeInfo, final DataStore dataStore) {
        ...
        final StoragePoolVO storagePool = this._storagePoolDao.findById(dataStore.getId());
        ...
        final VolumeVO volume = this._volumeDao.findById(volumeInfo.getId());

        details.put(DiskTO.PROTOCOL_TYPE, (volume.getPoolType() != null) ? volume.getPoolType().toString() : null);

        final ChapInfo chapInfo = this.volService.getChapInfo(volumeInfo, dataStore);
    ...
    @Override
    public void prepare(final VirtualMachineProfile vm, final DeployDestination dest) throws StorageUnavailableException,
    ...
    @DB
    protected VolumeVO switchVolume(final VolumeVO existingVolume, final VirtualMachineProfile vm) {
    ...
    public boolean canVmRestartOnAnotherServer(final long vmId) {
        final List<VolumeVO> vols = this._volsDao.findCreatedByInstance(vmId);
        for (final VolumeVO vol : vols) {
            final StoragePoolVO storagePoolVO = this._storagePoolDao.findById(vol.getPoolId());
    ...
    public String getStoragePoolOfVolume(final long volumeId) {
        ...
        return this.dataStoreMgr.getPrimaryDataStore(vol.getPoolId()).getUuid();
    }
    ...
    public void updateVolumeDiskChain(final long volumeId, final String path, final String chainInfo) {
        ...
        if (needUpdate) {
            ...
            this._volsDao.update(volumeId, vol);
        }
    }
    ...
    protected Volume liveMigrateVolume(final Volume volume, final StoragePool destPool) {
        final VolumeInfo vol = this.volFactory.getVolume(volume.getId());
        final AsyncCallFuture<VolumeService.VolumeApiResult> future = this.volService.migrateVolume(vol, (DataStore) destPool);
        try {
            final VolumeService.VolumeApiResult result = future.get();
            if (result.isFailed()) {
