...
public class Upgrade420to421 implements DbUpgrade {
    final static Logger s_logger = LoggerFactory.getLogger(Upgrade420to421.class);
    ...
    public File[] getPrepareScripts() {
        final String script = Script.findScript("", "db/schema-420to421.sql");
        if (script == null) {
    ...
    private void upgradeResourceCount(final Connection conn) {
        ...
        final String sqlSelectAccountIds = "select id, domain_id FROM `cloud`.`account` where removed is NULL ";
        final String sqlSelectOfferingTotals = "SELECT SUM(service_offering.cpu), SUM(service_offering.ram_size)"
        ...
        final String sqlSelectTotalVolumeSize =
        ...
        final String sqlSelectTotalPathlessVolumeSize =
                "SELECT sum(size) FROM `cloud`.`volumes` WHERE account_id= ?"
                        + " AND path is NULL AND state not in ('Allocated') AND removed is NULL";
        final String sqlSelectTotalSnapshotSize = "SELECT sum(size) FROM `cloud`.`snapshots` WHERE account_id= ? AND removed is NULL";
        final String sqlSelectTotalTemplateStoreSize = "SELECT sum(template_store_ref.size) FROM `cloud`.`template_store_ref`,`cloud`.`vm_template` WHERE account_id = ?"
                + " AND template_store_ref.template_id = vm_template.id AND download_state = 'DOWNLOADED' AND destroyed = false AND removed is NULL";
        final String sqlSelectDomainIds = "select id FROM `cloud`.`domain`";
        final String sqlSelectAccountCount = "select account.domain_id,sum(resource_count.count) from `cloud`.`account` left join `cloud`.`resource_count` on account" +
        ...
        try (
            ...
            try (ResultSet rsDomainIds = pstmtSelectDomainIds.executeQuery()) {
                while (rsDomainIds.next()) {
                    final long domain_id = rsDomainIds.getLong(1);
            ...
            for (final String resource_type : resource_types) {
                ...
                try (ResultSet rsAccountCount = pstmtSelectAccountCount.executeQuery()) {
                    while (rsAccountCount.next()) {
                        final long domain_id = rsAccountCount.getLong(1);
                        final long resource_count = rsAccountCount.getLong(2);
    ...
    private void updateOverprovisioningPerVm(final Connection conn) {
        ...
        try (PreparedStatement selectConfiguration = conn.prepareStatement("select value from `cloud`.`configuration` where name=?")) {
            ...
            try (
                    PreparedStatement pstmt2 = conn
                            .prepareStatement("select id, hypervisor_type from `cloud`.`vm_instance` where removed is null and id not in (select vm_id from  `cloud`" +
                                    ".`user_vm_details` where name='cpuOvercommitRatio')");
    ...
    private static void upgradeResourceCountforAccount(final Connection conn, final Long accountId, final Long domainId, final String type, final Long resourceCount) throws
        ...
        final String sqlInsertResourceCount = "INSERT INTO `cloud`.`resource_count` (account_id, type, count) VALUES (?,?,?) ON DUPLICATE KEY UPDATE id=LAST_INSERT_ID(id), " +
                "count=?";
    ...
    private static void upgradeResourceCountforDomain(final Connection conn, final Long domainId, final String type, final Long resourceCount) throws SQLException {
        //update or insert into resource_count table.
        final String sqlInsertResourceCount = "INSERT INTO `cloud`.`resource_count` (domain_id, type, count) VALUES (?,?,?) ON DUPLICATE KEY UPDATE id=LAST_INSERT_ID(id), count=?";
        try (PreparedStatement pstmt = conn.prepareStatement(sqlInsertResourceCount)) {
