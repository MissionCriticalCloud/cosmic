...
public class Upgrade452to460 implements DbUpgrade {
    ...
    private void updateSystemVmTemplates(final Connection conn) {
        ...
        final Set<Hypervisor.HypervisorType> hypervisorsListInUse = new HashSet<>();
                ...
                if (templateId != -1) {
                    ...
                    if (hypervisorsListInUse.contains(hypervisorAndTemplateName.getKey())) {
                        ...
                        try (PreparedStatement update_templ_url_pstmt = conn
                                .prepareStatement("UPDATE `cloud`.`vm_template` SET url = ? , checksum = ? WHERE hypervisor_type = ? AND type = 'SYSTEM' AND removed is null " +
