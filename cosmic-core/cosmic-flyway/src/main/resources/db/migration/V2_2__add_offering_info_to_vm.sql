-- Add offering info to vm
ALTER TABLE vm_instance
  ADD cpunumber INT(10) UNSIGNED;
ALTER TABLE vm_instance
  ADD ramsize BIGINT(20) UNSIGNED;

-- Change cpu and ram_size fields to retrieve from vm_instance if configured (>0)
ALTER VIEW `user_vm_view`
  AS
    SELECT
      ...
      (CASE WHEN `vm_instance`.`cpunumber` > 0
        THEN `vm_instance`.`cpunumber`
      ...
      (CASE WHEN `vm_instance`.`ramsize` > 0
        THEN `vm_instance`.`ramsize`
    ...
    FROM ((((((((((((((((((((((((((((((`user_vm`
      JOIN `vm_instance`                            ON (((`vm_instance`.`id` = `user_vm`.`id`) AND isnull(`vm_instance`.`removed`))))
