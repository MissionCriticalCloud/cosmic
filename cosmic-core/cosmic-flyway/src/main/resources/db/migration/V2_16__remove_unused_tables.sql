...
DROP VIEW IF EXISTS `security_group_view`;

-- Rename unused colums data_center table
ALTER TABLE `data_center`
  DROP COLUMN `is_security_group_enabled`;

-- Rename unused colums hypervisor_capabilities table
ALTER TABLE `hypervisor_capabilities`
  DROP COLUMN `security_group_enabled`;

-- Rename unused colums hypervisor_capabilities table
ALTER TABLE `physical_network_service_providers`
  DROP COLUMN `security_group_service_provided`;

-- Recreate data_center_view
DROP VIEW IF EXISTS `data_center_view`;
CREATE VIEW `data_center_view` AS
  ...
  FROM (((`cloud`.`data_center`
    LEFT JOIN `cloud`.`domain` ON ((`cloud`.`data_center`.`domain_id` = `cloud`.`domain`.`id`))) LEFT JOIN `cloud`.`dedicated_resources`
      ON ((`cloud`.`data_center`.`id` = `cloud`.`dedicated_resources`.`data_center_id`))) LEFT JOIN `cloud`.`affinity_group`
      ON ((`cloud`.`dedicated_resources`.`affinity_group_id` = `cloud`.`affinity_group`.`id`)));
...
CREATE VIEW `async_job_view` AS
  ...
  FROM ((((((((((((((((((`async_job`
    LEFT JOIN `account` ON ((`async_job`.`account_id` = `account`.`id`))) LEFT JOIN `domain` ON ((`domain`.`id` = `account`.`domain_id`))) LEFT JOIN `user`
      ...
      ON ((`async_job`.`instance_id` = `storage_pool`.`id`))) LEFT JOIN `user_ip_address` ON ((`async_job`.`instance_id` = `user_ip_address`.`id`))) LEFT JOIN `physical_network`
      ON ((`async_job`.`instance_id` = `physical_network`.`id`))) LEFT JOIN `physical_network_traffic_types` ON ((`async_job`.`instance_id` = `physical_network_traffic_types`.`id`))) LEFT JOIN
    `physical_network_service_providers` ON ((`async_job`.`instance_id` = `physical_network_service_providers`.`id`))) LEFT JOIN `firewall_rules`
      ON ((`async_job`.`instance_id` = `firewall_rules`.`id`))) LEFT JOIN `account` `acct` ON ((`async_job`.`instance_id` = `acct`.`id`))) LEFT JOIN `user` `us`
      ON ((`async_job`.`instance_id` = `us`.`id`))) LEFT JOIN `static_routes` ON ((`async_job`.`instance_id` = `static_routes`.`id`))) LEFT JOIN `vpc_gateways`
      ON ((`async_job`.`instance_id` = `vpc_gateways`.`id`)));
