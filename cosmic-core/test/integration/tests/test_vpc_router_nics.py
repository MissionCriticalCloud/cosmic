...
class TestVPCNics(cosmicTestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def deployvm_in_network(self, network):
        try:
            ...
            return vm
        except CloudStackApiException as _e:
            ...
            for vm in o.get_vms():
                try:
                    ...
                    self.logger.debug("Checking if we can SSH into VM=%s on public_ip=%s" %
                                      (virtual_machine.name, public_ip.ipaddress))

                    virtual_machine.get_ssh_client(ipaddress=public_ip.ipaddress)
