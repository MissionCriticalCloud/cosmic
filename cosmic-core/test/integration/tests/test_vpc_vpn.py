...
class TestVpcVpn(cosmicTestCase):
    ...
    def test_01_vpc_remote_access_vpn(self):
        ...
        self.logger.debug("VM %s deployed in VPC %s" % (vm.id, vpc.id))
    ...
    def _test_vpc_site2site_vpn(self, vpc_offering, num_vpcs=3):
        ...
        for i in range(num_vpcs):
            ...
            for key in vpcservice_n.keys():
                vpcservice_n[key] = vpcservice_n[key].format(N=repr(i))
        ...
        for i in range(num_vpcs):
            ...
            for key in ntwk_info_n.keys():
                ntwk_info_n[key] = ntwk_info_n[key].format(N=repr(i))
        ...
        for i in range(num_vpcs):
            vm_n = VirtualMachine.create(self.apiclient, services=self.services["virtual_machine"],
                                         ...
                                         mode='advanced' if (i == 0) or (i == maxnum_vm) else 'default'
                                         )
        ...
        for i in range(num_vpcs):
            ...
            customer_response = VpnCustomerGateway.create(self.apiclient, services, "Peer VPC" + repr(i), ip.ipaddress, vpc_list[i].cidr, self.account.name, self.domain.id)
        ...
        self.assertEqual(self.routers_in_right_state(vpcid=vpc_list[maxnum_vm].id), True, "Check whether the routers are in the right state.")

        # 6) Connect VPCi with VPC0
        for i in range(num_vpcs)[1:]:
            passive_vpn = Vpn.createVpnConnection(self.apiclient, vpn_cust_gw_list[0].id, vpn_response_list[i]['id'], True)
            ...
            time.sleep(15)
            self.logger.debug("Resetting VPN connection with id %s" % (passive_vpn['id']))
            Vpn.resetVpnConnection(self.apiclient, passive_vpn['id'])
            ...
            if ssh_client:
                # run ping test
                for j in range(num_vpcs)[1:]:
                    _packet_loss = ssh_client.execute("/bin/ping -c 3 -t 10 " + vm_list[j].nic[0].ipaddress)
                    self.logger.debug("Ping from vm0 to vm%d to make sure tunnel is up." % j)
            else:
                self.fail("Failed to setup ssh connection to %s" % vm_list[0].public_ip)

            self.logger.debug("Waiting for the VPN with id %s to connect" % (passive_vpn['id']))
            max_retries = 30
            retries = 0
            while retries < max_retries:
                vpnconn2_response = Vpn.listVpnConnection(self.apiclient, listall=True, id=passive_vpn['id'])
                if len(vpnconn2_response) > 0 and vpnconn2_response[0].state.lower() == 'connected':
                    break
                retries += 1
                time.sleep(2)
            self.assertLess(retries, max_retries, "Failed to reconnect VPN with id %s" % (passive_vpn['id']))

        # First the last VM
        # setup ssh connection to vm maxnumVM
        self.logger.debug("Setup SSH connection to last VM created (%d) to ensure availability for ping tests" % maxnum_vm)
        ssh_max_client = vm_list[maxnum_vm].get_ssh_client(retries=20)
        self.assertIsNotNone(ssh_max_client, "Failed to setup SSH to last VM created (%d)" % maxnum_vm)
        ...
        if ssh_client:
            ...
            for i in range(num_vpcs)[1:]:
                ...
                self.assertEquals(int(packet_loss), 0, " Ping towards vm" + repr(i) + " did not succeed")
