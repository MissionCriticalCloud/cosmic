...
class TestPasswordService(cosmicTestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def perform_password_service_tests(self, vpc_off):
        ...
        for router in routers:
            if len(routers) == 1 or router.redundantstate == 'MASTER':
                self._perform_password_service_test(router, network_1)
        self.enable_template_password(
            template_id=self.template.id,
    ...
    def routers_in_right_state(self):
        ...
        while test_tries < max_tries:
            ...
            if master_found and backup_found:
                self.logger.debug("Found router in MASTER and in BACKUP state so continuing")
                break
            test_tries += 1
    ...
    def _perform_dnsmasq_service_dns_test(self, router, network):
        ...
        for dns_servers in self.services["dns_servers"]:
            ...
            if not self._test_dnsmasq_service_dhcp_dns_config(router, dns_servers):
    ...
    def _test_process_running(self, find_process, router):
        host = self.get_host_details(router)
    ...
    def _test_password_server_logs(self, vm, router):
        host = self.get_host_details(router)
    ...
    def _test_dnsmasq_service_dhcp_dns_config(self, router, dns_server):
        host = self.get_host_details(router)
    ...
    def createVPC(self, vpc_offering, cidr='10.1.1.1/16'):
        try:
            ...
            if self.routers_in_right_state():
                self.logger.debug("Routers are in the correct state")
            else:
                self.logger.debug("Routers are in incorrect state!")
        except Exception as e:
    ...
    def createACL(self, vpc):
        ...
        createAclCmd = {
            'name': "ACL-Test-%s" % vpc.id,
            'description': "ACL-Test-%s" % vpc.id,
            'vpcid': vpc.id,
            'fetch_result': True
        }
        try:
            acl = self.apiclient.createNetworkACLList(**createAclCmd)
            self.assertIsNotNone(acl['networkacllist']['id'], "Failed to create ACL.")

            self.logger.debug("Created ACL with ID: %s" % acl['networkacllist']['id'])
        except Exception as e:
            self.fail('Unable to create ACL due to %s ' % e)

        return type("ACL", (object,), acl['networkacllist'])
    ...
    def createACLItem(self, aclId, cidr="0.0.0.0/0"):
        ...
        try:
            ...
            self.assertIsNotNone(aclItem['networkacl']['id'], "Failed to create ACL item.")

            self.logger.debug("Created ACL Item ID: %s" % aclItem['networkacl']['id'])
        except Exception as e:
