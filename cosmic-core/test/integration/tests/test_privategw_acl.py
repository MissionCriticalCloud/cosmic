...
class TestPrivateGateway(cosmicTestCase):
    ...
    def setup_infra(cls, redundant=False):
        ...
        cls.check_vpc_state(cls.vpc1)
        cls.logger.debug("VPC '%s' is in correct state", cls.vpc1.name)
        ...
        cls.check_vpc_state(cls.vpc2)
        cls.logger.debug("VPC '%s' is in correct state", cls.vpc2.name)
        ...
        cls.logger.debug("Public IP '%s' acquired, VPC: %s, Network: %s", cls.public_ip2.ipaddress, cls.vpc2.name, cls.network2.name)
    ...
    @staticmethod
    def check_vpc_state(cls, retries=10):
        while retries > 0:
            if cls.is_state_ok():
                break
            time.sleep(5)
            retries -= 1
        if retries == 0:
            raise Exception("Router(s) are not in the correct state")

    def _test_connectivity(self):

        try:
            ping_count = 3
            ssh_client = self.vm1.get_ssh_client(ipaddress=self.public_ip1.ipaddress, reconnect=True)
    ...
    def define_custom_acl(self):
        ...
        try:
            command = {
                'aclid': acl1.id,
                'gatewayid': self.private_gateway1.id,
                'fetch_result': True
            }
        ...
        try:
            command2 = {
                'aclid': acl2.id,
                'gatewayid': self.private_gateway2.id,
                'fetch_result': True
            }
            response2 = Response(self.api_client.replaceNetworkACLList(**command2))
