...
class TestVPCRedundancy(cosmicTestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def query_routers(self, count=2, showall=False):
        ...
        self.logger.debug('query_routers routers: %s' % " ".join([x.name for x in self.routers]))
        self.assertEqual(
    ...
    def check_routers_state(self, count=2, status_to_check="MASTER", expected_count=1, showall=False):
        ...
        self.logger.debug("router.redundant.vrrp.interval is ==> %s" % vrrp_interval[0].value)
        ...
        for router in self.routers:
            if router.state == "Running":
                ...
                for _ in range(5):
                    user, passwd = get_host_credentials(self.config, host.name)
    ...
    def stop_router(self, router):
        ...
        cmd = {'id': router.id, 'forced': True, 'fetch_result': True}
        self.apiclient.stopRouter(**cmd)

    def reboot_router(self, router):
        self.logger.debug('Rebooting router %s' % router.id)
        cmd = {'id': router.id, 'fetch_result': True}
    ...
    def stop_router_by_type(self, type):
        ...
        for router in self.routers:
            self.logger.debug('Checking state of router %s' % router.name)
            if router.redundantstate == type:
                self.logger.debug('Stopping router %s' % router.name)
    ...
    def reboot_router_by_type(self, type):
        ...
        for router in self.routers:
            self.logger.debug('Checking state of router %s' % router.name)
            if router.redundantstate == type:
                self.logger.debug('Rebooting router %s' % router.name)
    ...
    def destroy_routers(self):
        ...
        for router in self.routers:
            self.stop_router(router)
            cmd = {'id': router.id, 'fetch_result': True}
            self.logger.debug('Destroying router %s' % router.name)
    ...
    def create_network(self, network_offering, gateway='10.1.1.1', vpc=None, nr_vms=2):
        ...
        try:
            ...
            self.fail('Unable to create a Network with offering=%s because of %s ' % (network_offering.id, e.error['errortext']))
        except Exception as e:
    ...
    def check_ssh_into_vm(self, vm, public_ip, expectFail=False, retries=5):
        self.logger.debug("Checking if we can SSH into VM=%s on public_ip=%s (%r)" %
                          (vm.name, public_ip.ipaddress, expectFail))
        vm.ssh_client = None
        try:
            if 'retries' in inspect.getargspec(vm.get_ssh_client).args:
                vm.get_ssh_client(ipaddress=public_ip.ipaddress, retries=retries)
            else:
                vm.get_ssh_client(ipaddress=public_ip.ipaddress)
            if expectFail:
                self.fail("SSH into VM=%s on public_ip=%s is successful (Not Expected)" %
                          (vm.name, public_ip.ipaddress))
            else:
                self.logger.debug("SSH into VM=%s on public_ip=%s is successful" %
                                  (vm.name, public_ip.ipaddress))
        except:
            if expectFail:
                self.logger.debug("Failed to SSH into VM - %s (Expected)" % (public_ip.ipaddress))
            else:
                self.fail("Failed to SSH into VM - %s" % (public_ip.ipaddress))
        ...
        for o in self.networks:
            for vmObj in o.get_vms():
                ...
                try:
                    ...
                    self.logger.debug("SSH into VM: %s" % public_ip.ipaddress)

                    ssh = vm.get_ssh_client(ipaddress=public_ip.ipaddress)
