...
public class TrafficSentinelResource implements ServerResource {
    ...
    public StartupCommand[] initialize() {
        final StartupTrafficMonitorCommand cmd = new StartupTrafficMonitorCommand();
        cmd.setName(_name);
    ...
    private Answer execute(final MaintainCommand cmd) {
        return new MaintainAnswer(cmd);
    }

    private synchronized DirectNetworkUsageAnswer execute(final DirectNetworkUsageCommand cmd) {
        try {
    ...
    private synchronized RecurringNetworkUsageAnswer execute(final RecurringNetworkUsageCommand cmd) {
        return new RecurringNetworkUsageAnswer(cmd);
    }

    private DirectNetworkUsageAnswer getPublicIpBytesSentAndReceived(final DirectNetworkUsageCommand cmd) throws ExecutionException {
        final DirectNetworkUsageAnswer answer = new DirectNetworkUsageAnswer(cmd);

        try {
            //Direct Network Usage
            final URL trafficSentinel;
            ...
            try {
                ...
                final BufferedReader in = new BufferedReader(new InputStreamReader(trafficSentinel.openStream()));
                ...
                while ((inputLine = in.readLine()) != null) {
                    ...
                    if (st.countTokens() == 3) {
                        final String publicIp = st.nextToken();
                        final Long bytesSent = new Long(st.nextToken());
                        final Long bytesRcvd = new Long(st.nextToken());
                        final long[] bytesSentAndReceived = new long[2];
    ...
    private String getScript(final List<String> ips, final Date start, final Date end) {
        String IpAddresses = "";
        ...
        final String startDate = getDateString(start);
        final String endtDate = getDateString(end);
    ...
    private String getDateString(final Date date) {
        final DateFormat dfDate = new SimpleDateFormat("yyyyMMdd HH:mm:ss");
